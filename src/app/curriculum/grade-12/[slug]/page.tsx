...
import {
  GRADE_12_CURRICULUM,
  getGrade12UnitBySlug,
  type Grade12VocabItem,
} from "@/lib/curriculum/grade12-data";
...
export default function Grade12UnitDetailPage() {
  ...
  const unit = useMemo(() => (slug ? getGrade12UnitBySlug(slug) : undefined), [slug]);
  const { user } = useRole();
  ...
  const unitIndex = useMemo(() => {
    if (!unit) return -1;
    return GRADE_12_CURRICULUM.findIndex((u) => u.id === unit.id);
  }, [unit]);
  const prevUnit = unitIndex > 0 ? GRADE_12_CURRICULUM[unitIndex - 1] : null;
  const nextUnit = unitIndex >= 0 && unitIndex < GRADE_12_CURRICULUM.length - 1 ? GRADE_12_CURRICULUM[unitIndex + 1] : null;
  ...
  useEffect(() => {
    ...
    const savedCard = localStorage.getItem(`lingoquest_last_card_g12_${slug}`);
    if (savedCard) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && slug && currentCardIndex >= 0) {
      localStorage.setItem(`lingoquest_last_card_g12_${slug}`, currentCardIndex.toString());
    }
    ...
    return (
      <AppShell>
        <div className="mx-auto max-w-xl py-16 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">Đường dẫn bài học Lớp 12 không tồn tại hoặc đã bị đổi.</p>
          <div className="mt-6">
            <Button asChild className="rounded-xl bg-purple-600 hover:bg-purple-700 text-white">
              <Link href="/curriculum/grade-12">Về danh mục Tiếng Anh 12</Link>
            </Button>
          </div>
        </div>
      </AppShell>
  ...
  const currentFlashcard: Grade12VocabItem | undefined = unit.vocabulary[currentCardIndex];
    ...
    <AppShell>
      <div className="mx-auto max-w-7xl pb-16">
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <GradeSwitcher currentGrade={12} compact />
        </div>

        {/* Navigation Breadcrumb & Unit Switcher */}
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
            <Link href="/curriculum" className="hover:text-purple-600 transition-colors">
              Chương trình
            </Link>
            <ChevronRight className="h-3 w-3 text-slate-400" />
            <Link href="/curriculum/grade-12" className="hover:text-purple-600 transition-colors">
              Lớp 12
            </Link>
            <ChevronRight className="h-3 w-3 text-slate-400" />
          ...
          <div className="flex items-center gap-2">
            {prevUnit && (
              <Button asChild variant="outline" size="sm" className="rounded-xl border-slate-200 text-xs font-bold shadow-2xs">
                <Link href={`/curriculum/grade-12/${prevUnit.slug}`}>
            ...
            <select
              ...
              value={unit.slug}
              onChange={(e) => {
                router.push(`/curriculum/grade-12/${e.target.value}`);
              }}
              className="text-xs font-bold text-slate-700 bg-white border border-slate-200 rounded-xl px-2.5 py-1.5 shadow-2xs hover:border-purple-400 focus:outline-none focus:ring-2 focus:ring-purple-500/20 cursor-pointer"
            >
              {GRADE_12_CURRICULUM.map((u) => (
            ...
            {nextUnit && (
              <Button asChild size="sm" className="rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold shadow-2xs">
                <Link href={`/curriculum/grade-12/${nextUnit.slug}`}>
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-950 via-purple-900 to-slate-900 p-8 text-white shadow-xl mb-8"
        >
          ...
          <div className="relative z-10 flex flex-col justify-between gap-6 md:flex-row md:items-center">
            <div className="max-w-2xl">
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <span className="inline-flex items-center gap-1 rounded-full bg-purple-500/20 px-3 py-0.5 text-xs font-bold text-purple-200 border border-purple-400/30">
                  {unit.isReview ? "Review Ôn tập" : `Tiếng Anh 12 • Unit ${unit.unitNumber}`}
                </span>
              ...
              <p className="mt-1 text-sm text-purple-200 font-medium">{unit.titleVi}</p>
            ...
            {unit.vocabulary.length > 0 && (
              <div className="rounded-2xl border border-white/15 bg-white/10 p-4 backdrop-blur-md shrink-0 sm:min-w-[240px]">
                <div className="flex items-center justify-between text-xs font-semibold mb-2">
                  <span className="text-purple-100">Tiến độ từ vựng:</span>
                ...
                <div className="grid grid-cols-3 gap-2 text-center text-[10px]">
                  <div className="rounded-lg bg-black/20 p-1.5">
                    <span className="block font-bold text-purple-300">{stats.mastered}</span>
            ...
            {unit.vocabulary.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-12 text-center">
                ...
                <p className="mt-1 text-xs text-slate-500 max-w-md mx-auto">
                  Bài Review này tổng hợp kiến thức từ các Units trước để chuẩn bị cho kỳ thi. Bạn hãy xem phần Ngữ pháp hoặc luyện tập các dạng bài đọc hiểu!
                </p>
                <div className="mt-6">
                  <Button onClick={() => handleTabChange("grammar")} className="rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold">
                  ...
                  {!flashcardMode && (
                    <div className="relative w-full sm:w-72">
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-slate-200 bg-white py-2 pl-9 pr-3 text-xs font-medium text-slate-900 placeholder:text-slate-400 focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20 transition-all shadow-xs"
                      />
                    </div>
                  )}
                </div>
                    ...
                    <div className="lg:col-span-2">
                      <div className="rounded-3xl border border-slate-200/90 bg-white p-6 sm:p-8 shadow-md">
                        ...
                        <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
                          <div className="flex items-center gap-2">
                            <span className="rounded-full bg-purple-50 px-2.5 py-1 text-xs font-extrabold text-purple-700">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {hideMeaningForRecall ? <EyeOff className="h-4 w-4 text-purple-600" /> : <Eye className="h-4 w-4" />}
                            </button>
                          </div>
                        </div>
                          ...
                          <div className="mt-4 min-h-[48px] flex items-center justify-center">
                            {hideMeaningForRecall ? (
                              <button
                                ...
                                className="rounded-xl border border-dashed border-purple-300 bg-purple-50/50 px-4 py-2 text-xs font-bold text-purple-700 hover:bg-purple-100/50 transition-colors"
                              >
                        ...
                        <div className="mt-4 flex flex-wrap items-center justify-center gap-3 border-y border-slate-100 py-4">
                          <Button
                            ...
                            className="rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs shadow-xs"
                          >
                          ...
                          <Button
                            ...
                            variant="outline"
                            className="rounded-xl border-purple-200 text-purple-700 hover:bg-purple-50 text-xs font-bold"
                          >
                          ...
                          <div className="mt-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {cardCollocations.map((colloc, cIdx) => (
                                <span
                                  key={cIdx}
                                  className="rounded-lg bg-purple-50/70 border border-purple-100 px-2.5 py-1 text-xs font-semibold text-purple-800"
                                >
                                  {colloc}
                                </span>
                              ))}
                ...
                {!flashcardMode && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {filteredVocab.map((item, idx) => (
                      <div
                        key={item.id}
                        className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs hover:border-purple-300 transition-all flex flex-col justify-between"
                      >
                        <div>
                          ...
                          <p className="font-mono text-xs font-semibold text-purple-600 mb-1">{item.ipa}</p>
                ...
                {unit.sections.map((sec, sIdx) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="flex h-5 w-5 items-center justify-center rounded-full bg-purple-100 text-[10px] font-black text-purple-700">
                        {sIdx + 1}
                      </span>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-6">
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="rounded-full bg-violet-600 text-white text-[10px] font-black px-2.5 py-0.5 uppercase tracking-wider">
                    Đề kiểm tra 50 câu
                  </span>
                  <span className="text-[10px] font-bold text-violet-700 bg-violet-50 px-2 py-0.5 rounded-md">
                    Gộp Toàn diện Từ vựng &amp; Ngữ pháp
                  </span>
                </div>
              ...
              {quizSubmitted ? (
                ...
                <Button
                  ...
                  className="rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-xs font-bold shadow-xs py-2.5 px-4"
                >
              ...
              <div className="flex flex-wrap gap-1.5">
                {quizQuestions.map((q, idx) => {
                  ...
                  if (quizSubmitted) {
                    ...
                    color = "bg-violet-600 text-white font-bold";
                  }
                  ...
                  return (
                    <button
                      key={idx}
                      onClick={() => {
                        const el = document.getElementById(`g12-question-${q.id}`);
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono text-xs font-semibold text-purple-600">{q.ipa}</span>
                          <span className="rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-600 uppercase">
                            {q.pos}
                          </span>
                        </div>
