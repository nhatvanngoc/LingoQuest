...
export default function Grade12CurriculumPage() {
  const { user } = useRole();
  ...
  const filteredUnits = useMemo(() => {
    return GRADE_12_CURRICULUM.filter((u) => {
  ...
  const totalVocab = useMemo(() => {
    return GRADE_12_CURRICULUM.reduce((acc, u) => acc + (u.vocabulary?.length || 0), 0);
  }, []);
    ...
    <AppShell>
      <div className="mx-auto max-w-7xl pb-16">
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-950 via-purple-900 to-slate-900 p-8 text-white shadow-xl md:p-10 mb-8"
        >
          {/* Background Decorative Rings */}
          <div className="pointer-events-none absolute -right-20 -top-20 h-80 w-80 rounded-full bg-purple-500/20 blur-3xl" />
          <div className="pointer-events-none absolute -bottom-20 right-40 h-60 w-60 rounded-full bg-violet-400/15 blur-2xl" />

          <div className="relative z-10 max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-purple-400/30 bg-purple-500/20 px-3.5 py-1 text-xs font-semibold text-purple-200 backdrop-blur-md mb-4">
              <GraduationCap className="h-4 w-4 text-purple-300" />
              Chương trình chuẩn Bộ GD&ĐT 2018 • Tiếng Anh 12 Global Success
            </div>

            <h1 className="font-heading text-3xl font-extrabold tracking-tight sm:text-4xl text-white">
              Lộ trình Tiếng Anh Lớp 12 Về đích Toàn diện
            </h1>
            <p className="mt-3 text-base text-purple-100/90 leading-relaxed sm:text-lg">
              Đầy đủ 10 Units bứt phá và 4 bài Review ôn thi tốt nghiệp THPT. Tích hợp từ vựng học thuật B2+,
              phương pháp phản xạ âm thanh Deep Imprint Flashcard, luyện phát âm AI Speech Checker, ngữ pháp phân hóa cao và bộ đề thực chiến.
            </p>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
              <div className="rounded-2xl border border-white/10 bg-white/10 p-3.5 backdrop-blur-md">
                ...
                <span className="block text-xs font-medium text-purple-200">Units Trọng tâm</span>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/10 p-3.5 backdrop-blur-md">
                <span className="text-2xl font-black text-white">{totalVocab}+</span>
                <span className="block text-xs font-medium text-purple-200">Từ vựng & Audio</span>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/10 p-3.5 backdrop-blur-md">
                <span className="text-2xl font-black text-white">14</span>
                <span className="block text-xs font-medium text-purple-200">Chuyên đề ngữ pháp</span>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/10 p-3.5 backdrop-blur-md">
                <span className="text-2xl font-black text-white">B2 - B2+</span>
                <span className="block text-xs font-medium text-purple-200">Chuẩn CEFR</span>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-1.5 rounded-2xl border border-slate-200/80 bg-white p-1.5 shadow-xs">
            ...
            >
              Tất cả ({GRADE_12_CURRICULUM.length})
            </button>
            <button
          ...
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-xs font-medium text-slate-900 placeholder:text-slate-400 focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20 transition-all shadow-xs"
            />
          </div>
        </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  {/* Title */}
                  <Link href={`/curriculum/grade-12/${unit.slug}`}>
                    <h2 className="font-heading text-lg font-bold text-slate-900 group-hover:text-purple-600 transition-colors">
                      {unit.titleEn}
                    </h2>
                  </Link>
                  ...
                  {unit.vocabulary.length > 0 && (
                    <div className="mt-4 rounded-xl bg-slate-50/80 p-2.5 border border-slate-100">
                      <div className="flex items-center justify-between text-[11px] font-semibold text-slate-600 mb-1">
                        ...
                        <span className={stats.percent > 0 ? "text-purple-700 font-bold" : "text-slate-500"}>
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 flex items-center gap-2">
                  <Button
                    asChild
                    className="w-full justify-between rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs shadow-xs"
                  >
                    <Link href={`/curriculum/grade-12/${unit.slug}`}>
